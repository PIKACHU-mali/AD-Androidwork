...
public interface LinkRepository extends JpaRepository<LinkRequest, String> {

	@Query("SELECT l FROM LinkRequest l WHERE l.counsellorUser = :counsellor AND l.journalUser = :journal")
	Optional<LinkRequest> findByCounsellorAndJournalUser(@Param("counsellor") CounsellorUser counsellor,
	                                                     @Param("journal") JournalUser journal);
    
	@Query("SELECT l FROM LinkRequest l WHERE l.journalUser = :j AND l.status = :s")
	List<LinkRequest> findByJournalUserAndStatus(@Param("j") JournalUser j, @Param("s") Status s);
    
	@Query("SELECT l FROM LinkRequest l WHERE l.counsellor = :c AND l.status = :s")
