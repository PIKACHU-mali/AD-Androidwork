package com.moodyclues.model;

import java.util.ArrayList;
import java.util.List;

import com.fasterxml.jackson.annotation.JsonIgnore;

import jakarta.persistence.CascadeType;
import jakarta.persistence.Entity;
import jakarta.persistence.ManyToMany;
import jakarta.persistence.OneToMany;
import jakarta.persistence.Table;

@Entity
@Table(name = "journal_users")
public class JournalUser extends User {

	private boolean showEmotion;
	
	@OneToMany(mappedBy = "user", cascade = CascadeType.ALL)
	@JsonIgnore
	private List<JournalEntry> entries = new ArrayList<>();
	
	@OneToMany(mappedBy = "user", cascade = CascadeType.ALL)
	@JsonIgnore
	private List<HabitsEntry> habitsEntries = new ArrayList<>();
	
	@ManyToMany(mappedBy = "clients", cascade = CascadeType.ALL)
	@JsonIgnore
	private List<CounsellorUser> counsellors = new ArrayList<>();
	
	@OneToMany(mappedBy = "journalUser")
	@JsonIgnore
	private List<LinkRequest> linkRequests = new ArrayList<>();
	
	// EMPTY CONSTRUCTOR
	public JournalUser() {
		
	}

	
	// GETTERS AND SETTERS BELOW
	public List<JournalEntry> getEntries() {
		return entries;
	}


	public boolean isShowEmotion() {
		return showEmotion;
	}


	public void setShowEmotion(boolean showEmotion) {
		this.showEmotion = showEmotion;
	}


	public void setEntries(List<JournalEntry> entries) {
		this.entries = entries;
	}


	public List<CounsellorUser> getCounsellors() {
		return counsellors;
	}


	public void setCounsellors(List<CounsellorUser> counsellors) {
		this.counsellors = counsellors;
	}


	public List<HabitsEntry> getHabitsEntries() {
		return habitsEntries;
	}


	public void setHabitsEntries(List<HabitsEntry> habitsEntries) {
		this.habitsEntries = habitsEntries;
	}


	public List<LinkRequest> getLinkRequests() {
		return linkRequests;
	}


	public void setLinkRequests(List<LinkRequest> linkRequests) {
		this.linkRequests = linkRequests;
	}
	
	

	



	

	
	
}
